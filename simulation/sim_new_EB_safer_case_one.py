    ...
    for time in experiment.study_days:
        ...
        if time==experiment.last_update_day+pd.DateOffset(days=global_policy_params.update_period):
            ...
            if global_policy_params.decision_times>200:
                ...
                print('global_info', time,global_policy_params.decision_times,time_module.strftime('%l:%M%p %Z on %b %d, %Y'),temp_params['like'],file=open('updates_case_one_global_{}_{}_{}.txt'.format(len(experiment.population),global_policy_params.update_period,sim_num), 'a'))
        ...
        for person in experiment.dates_to_people[time]:
                ...
                if time in participant.decision_times:
                    ...
                    #participant.steps = steps
                        print('p_info', time,global_policy_params.decision_times,time_module.strftime('%l:%M%p %Z on %b %d, %Y'),participant.pid,steps,participant.gid,file=open('updates_case_one_participant_{}_{}_{}.txt'.format(len(experiment.population),global_policy_params.update_period,sim_num), 'a'))
                    else:
                        #participant.steps_last_time_period = participant.steps
